<template>
  <v-dialog max-width="550" :activator="btn">
    <template v-slot:activator="{ props: activatorProps }">
      <v-btn v-bind="activatorProps" class="confirm-btn" rounded>Stwórz nową lekcję</v-btn>
    </template>
      ...
      <v-card class="dialog-card pa-8">
        ...
        <v-form @submit.prevent="submitLesson">
          ...
          <v-text-field
            ...
            v-model="title"
          ></v-text-field>
          <h6 class="mb-2">Typ</h6>
          <v-select
            :items="['Nauka wymowy', 'Gramatyka']"
            density="compact"
            v-model="lessonType"
            bg-color="white"
            rounded
          ...
          <v-btn type="submit" class="w-full confirm-btn" rounded> Stwórz nową lekcję </v-btn>
        </v-form>
      </v-card>
    </template>
  </v-dialog>
