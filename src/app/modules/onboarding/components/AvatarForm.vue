    ...
    <v-col cols="12" md="6" :key="set.id" v-for="set in avatarSets">
      <v-card
        ...
        :class="[
          ...
          set.id === avatarForm.selectedAvatarSet && 'card-active'
        ]"
        rounded="xl"
        @click="handleAvatarSetClick(set.id)"
      >
        <div class="selected-dot" v-if="set.id === avatarForm.selectedAvatarSet">
  ...
  <v-row class="w-full" v-if="uploadAvatar">
    <v-col cols="12" md="4">
      <v-card elevation="0" class="upload-card pa-8" rounded="lg" @click="triggerFileInput('dialogue')">
        <img v-if="avatarForm.dialogue" :src="avatarForm.dialogue" alt="dialogue-avatar" />
        <template v-if="!avatarForm.dialogue">
    ...
    <v-col cols="12" md="4">
      <v-card elevation="0" class="upload-card pa-8" rounded="lg" @click="triggerFileInput('mistake')">
        <img v-if="avatarForm.mistake" :src="avatarForm.mistake" alt="mistake-avatar" />
        <template v-if="!avatarForm.mistake">
          <v-icon class="upload-icon" icon="mdi-tray-arrow-up"></v-icon>
    ...
    <v-col cols="12" md="4">
      <v-card elevation="0" class="upload-card pa-8" rounded="lg" @click="triggerFileInput('congratulations')">
        <img v-if="avatarForm.congratulations" :src="avatarForm.congratulations" alt="congratulations-avatar" />
        <template v-if="!avatarForm.congratulations">
          <v-icon class="upload-icon" icon="mdi-tray-arrow-up"></v-icon>
...
const handleFileChange = (event: Event, type: string) => {
  ...
  switch (type) {
    case 'dialogue': {
      const newAvatarSet = { ...props.avatarForm, dialogue: fileURL };
      emit('update:avatars', newAvatarSet);
      break;
    }
    case 'mistake': {
      const newAvatarSet = { ...props.avatarForm, mistake: fileURL };
      emit('update:avatars', newAvatarSet);
      break;
    }
    case 'congratulations': {
      const newAvatarSet = { ...props.avatarForm, congratulations: fileURL };
      emit('update:avatars', newAvatarSet);
      break;
    }
  }
...
const handleAvatarSetClick = (id: number) => {
  const newAvatarSet = { ...props.avatarForm, selectedAvatarSet: id };
  emit('update:avatars', newAvatarSet);
};
</script>
